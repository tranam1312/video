...
class ImageSlideRenderer(gsTransition: GSTransition):GLSurfaceView.Renderer {
    ...
    fun changeTransition(context: Context,gsTransition: GSTransition) {
        mGSTransition = gsTransition
        val handle =  ShaderHelper.compileShader(GLES20.GL_FRAGMENT_SHADER, getFragmentShader(context,gsTransition.transitionCodeId))
    ...
    private fun getFragmentShader(context: Context,transitionCodeId: Int): String {
        val transitionCode = RawResourceReader.readTextFileFromRawResource(context, transitionCodeId)
